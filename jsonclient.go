...
import (
	...
	"flag"
	"fmt"
	"github.com/uzma012/Coronavirus-report"
	"net"
	"os"
	"time"
...
func main() {
	...
	defer conn.Close()
	fmt.Println("connected to covid service: ", addr)
	...
	for {
		...
		req := curr.covidRequest{Get: param}
		...
		// Display response
		var detail []curr1.covid
		err = json.NewDecoder(conn).Decode(&detail)
		if err != nil {
			switch err := err.(type) {
			case net.Error:
