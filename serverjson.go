...
// detail. The search result, a []curr.Currency, is marshalled
...
func main() {
	...
	defer ln.Close()
	log.Println("**** Pakistan Coronavirus-report ***")
...
func handleConnection(conn net.Conn) {
	...
	for {
		...
		if err := json.Unmarshal(buf, &req); err != nil {
			...
			cerr, jerr := json.Marshal(&curr.covidError{Error: err.Error()})
			if jerr != nil {
				log.Println("failed to marshal covidError:", jerr)
				continue
			}

			if _, werr := conn.Write(cerr); werr != nil {
				log.Println("failed to write to covidError:", werr)
				return
			}
			continue
		}

		// search detail, result is []curr.Currency
		result := curr.Find(detail, req.Get)
		...
		if err != nil {
			...
			if _, err := fmt.Fprintf(conn, `{"covid_error":"internal error"}`); err != nil {
